...
from geoclip_adv_attacks.attacks.SparsePatches import PGDTrim, PGDTrimKernel
from geoclip_adv_attacks.data.Im2GPS3k.download import load_im2gps_data
from geoclip_adv_attacks.training.CONFIG import CONFIG



def parse_args():
    args = {}
    # Run args from CONFIG
    args['seed'] = CONFIG['training']['seed']
    args['gpus'] = CONFIG['training']['gpus']
    args['force_cpu'] = False  # Default value
    
    # Data args from CONFIG
    args['dataset'] = CONFIG['data']['name']
    args['data_dir'] = CONFIG['paths']['data_dir']
    args['results_dir'] = CONFIG['paths']['results_dir']
    args['batch_size'] = CONFIG['data']['batch_size']
    args['report_info'] = CONFIG['training']['report_info']
    args['save_results'] = CONFIG['data']['save_results']
    args['l0_hist_limits'] = (0, 1, 2, 4, 8, 16, 32, 64, 128, 224, 256, 299, 512, 1024)
    
    # Model args
    args['model_name'] = ''  # Default value
    args['model_transform_input'] = True  # Default value
    
    # Attack args from CONFIG
    attack_config = CONFIG['attack']
    args['attack'] = 'PGDTrim'  # Default value
    args['eps_l_inf_from_255'] = int(attack_config['epsilon'] * 255)
    args['sparsity'] = attack_config['patch']['l0_limit']
    args['n_iter'] = attack_config['n_iter']
    args['n_restarts'] = attack_config['n_restarts']
    args['alpha'] = attack_config['alpha']
    args['att_init_zeros'] = not attack_config['patch']['rand_init']
    
    
    # PGDTrim specific args from CONFIG
    patch_config = attack_config['patch']
    args['att_dpo_dist'] = patch_config['att_dpo_dist']
    args['att_dpo_mean'] = patch_config['att_dpo_mean']
    args['att_dpo_std'] = patch_config['att_dpo_std']
    args['att_trim_steps'] = patch_config['att_trim_steps']
    args['att_max_trim_steps'] = patch_config['att_max_trim_steps']
    args['att_trim_steps_reduce'] = patch_config['att_trim_steps_reduce']
    args['att_const_dpo_mean'] = patch_config['att_const_dpo_mean']
    args['att_const_dpo_std'] = patch_config['att_const_dpo_std']
    args['att_post_trim_dpo'] = patch_config['att_post_trim_dpo']
    args['att_dynamic_trim'] = patch_config['att_dynamic_trim']
    args['att_mask_dist'] = patch_config['att_mask_dist']
    args['att_mask_prob_amp_rate'] = patch_config['att_mask_prob_amp_rate']
    args['att_norm_mask_amp'] = patch_config['att_norm_mask_amp']
    args['att_mask_opt_iter'] = patch_config['att_mask_opt_iter']
    args['att_n_mask_samples'] = patch_config['att_n_mask_samples']
    args['att_no_samples_limit'] = not patch_config['att_no_samples_limit']
    args['att_trim_best_mask'] = patch_config['att_trim_best_mask']
    args['att_kernel_size'] = patch_config['att_kernel_size']
    args['att_kernel_min_active'] = patch_config['att_kernel_min_active']
    args['att_kernel_group'] = patch_config['att_kernel_group']
...
def compute_data_args(args):
    ...
    if args.dataset == "Im2GPS3k":
        args.x_test, args.y_test = load_im2gps_data(args, transform=get_transforms())
...
def compute_models_args(args):
    args.model = GeoCLIP() # loads weights and everything in init
